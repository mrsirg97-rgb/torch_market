...
Key strengths:
- 30 instructions, 14 account types, 73 Kani formal verification proofs passed
- **V20.0.0 DeepPool migration**: All Raydium dependencies removed. Migration, vault swap, and fee swap now CPI into DeepPool — torch.market's own CPMM with auto-compounding swap fees (0.25%). Zero external AMM dependency. Pool validation reduced from 210 lines of raw byte parsing to simple PDA derivation. WSOL wrapping/unwrapping eliminated entirely
