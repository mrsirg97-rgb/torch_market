...
├── lib.rs              # Entry points and instruction routing (32 instructions as of v11.1.0)
...
├── kani_proofs.rs      # Formal verification proofs (cfg(kani), 71 proofs as of v11.0.1; math.rs unchanged in v11.1.0)
├── tests/litesvm/      # Integration tests via in-process LiteSVM (97 tests as of v11.1.0; covers Raydium CPI flow with fixture binaries)
...
| v11.1.0 | **Legacy-token unstick + lifecycle hygiene + liquidation cascade fix.** Migration handler now reads the active transfer-fee bps from the mint's `TransferFeeConfig` extension at the current epoch rather than the `TRANSFER_FEE_BPS` constant — unsticks pre-V11 mints whose stored bps differs from the current value (validated on TORCHY, 10-bps mint). Freeze-authority byte check now scoped to an inner block so the mint data borrow drops before the `SetAuthority(FreezeAccount, None)` CPI — fixes latent `AccountBorrowFailed` for mints with `freeze_authority = Some` (also surfaced via TORCHY). New `resolve_legacy_vote(result: bool)` admin instruction (32 ixs total) — one-shot tiebreaker for pre-V36 tokens stranded with `vote_finalized = false`; gated by `has_one = authority`, `bonding_complete`, `!vote_finalized`, `!migrated`. Defense-in-depth: `Buy` and `Sell` contexts now require `!bonding_curve.reclaimed` — uniformly blocks both sides on a reclaimed (effectively dead) token, forcing the `contribute_revival` path. Lifecycle: `repay` and `close_short` now refund rent and close the position PDA on full payoff (`is_full_repay` / `is_full_close`); pre-V11.1.0 the PDA was retained with zeroed fields. New `close_account_to(account, destination)` helper drains lamports, `resize(0)`, reassigns to system_program — verified Anchor 0.32 `exit_with_expected_owner` short-circuits via `is_closed()`. Cascade-safety: `liquidate` and `liquidate_short` no longer call `require_min_pool_liquidity`. New positions (`borrow`, `open_short`) still gate via `get_depth_max_ltv_bps` (returns 0 below `MIN_POOL_SOL_LENDING` → `PoolTooThin`); liquidations clear regardless of pool depth (bad debt is the alternative). New Rust litesvm test suite under `programs/torch_market/tests/litesvm/` (97 tests, loads Raydium CPMM SBF + mainnet amm_config / fee_receiver / WSOL fixtures, exercises full migration through Raydium CPI). 32 instructions total. 71 Kani proofs (unchanged — math.rs untouched). |
