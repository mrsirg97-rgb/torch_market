...
exports.fetchTokenRaw = exports.getTreasuryState = exports.getProtocolTreasuryState = exports.getUserStats = exports.getVaultWalletLink = exports.getVaultForWallet = exports.getVault = exports.getAllLoanPositions = exports.getShortPosition = exports.getLoanPosition = exports.getLendingInfo = exports.getMessages = exports.getHolders = exports.getToken = exports.getTokenMetadata = exports.getTokensPage = exports.getTokens = void 0;
...
const rawRpc = async (endpoint, method, params) => {
    ...
    if (json.error) {
        throw new Error(`RPC ${method} failed: ${json.error.message || 'unknown error'} (code ${json.error.code ?? '?'})`);
    }
    return json.result;
};
const decodeRawTokensFromAccounts = (accounts) => {
    const coder = new anchor_1.BorshCoder(torch_market_json_1.default);
    const tokens = [];
    for (const acc of accounts) {
        try {
            ...
            if (constants_1.BLACKLISTED_MINTS.includes(mintStr))
                continue;
            tokens.push({ mint: mintStr, bondingCurve: decoded });
        }
        catch {
 ...
 */
const getTokensPage = async (connection, params = {}) => {
    ...
    if (params.paginationKey)
        config.paginationKey = params.paginationKey;
    ...
    const result = await rawRpc(connection.rpcEndpoint, 'getProgramAccountsV2', [
        constants_1.PROGRAM_ID.toBase58(),
        config,
    ]);
    ...
    const summaries = rawTokens.map(toTokenSummary);
    return {
        tokens: summaries,
...
exports.getTokensPage = getTokensPage;
/**
...
const DEPTH_TIER_2 = 200000000000; // 200 SOL
const DEPTH_TIER_3 = 500000000000; // 500 SOL
...
const getDepthMaxLtvBps = (poolSol) => {
    if (poolSol < MIN_POOL_SOL_LENDING)
        return 0;
    if (poolSol < DEPTH_TIER_1)
        return DEPTH_LTV_0;
    if (poolSol < DEPTH_TIER_2)
        return DEPTH_LTV_1;
    if (poolSol < DEPTH_TIER_3)
        return DEPTH_LTV_2;
    return DEPTH_LTV_3;
};
/**
...
const getLendingInfo = async (connection, mintStr) => {
    ...
    try {
        ...
        poolSol = raydium.isWsolToken0
            ? Number(vault0Info.value.amount)
            : Number(vault1Info.value.amount);
    }
    catch {
...
const getLoanPosition = async (connection, mintStr, walletStr) => {
    ...
    try {
        ...
        if (raydium.isWsolToken0) {
            poolSol = vault0Amount;
            const tokenReserves = vault1Amount;
            if (tokenReserves > 0) {
                collateralValueSol = (collateral * poolSol) / tokenReserves;
            }
        }
        else {
            poolSol = vault1Amount;
...
const getShortPosition = async (connection, mintStr, walletStr) => {
    ...
    try {
        ...
        if (raydium.isWsolToken0) {
            ...
            const tokenReserves = vault1Amount;
            if (tokenReserves > 0) {
                debtValueSol = (totalOwedTokens * poolSol) / tokenReserves;
            }
        }
        else {
            poolSol = vault1Amount;
