...
Every token on Torch gets a bonding curve for price discovery, a treasury funded by a dynamic 17.5%→2.5% SOL rate, a 300M token lending reserve locked at creation, and a full margin system (lending + short selling) that activates after migration to DeepPool (Torch's in-house CPMM — no external DEX dependency). 100% of tokens go to buyers — no vote vault, no splits. The SDK builds transactions locally from the on-chain Anchor IDL, reads all state directly from Solana RPC, and handles routing between bonding curves and DeepPool automatically. No API server. No middleman.
...
This package lives in the [torch_market](https://github.com/mrsirg97-rgb/torch_market) workspace alongside the on-chain Anchor program. The IDL the SDK consumes is generated by `anchor build` from `programs/torch_market/` in the same repo — no manual sync, no version drift between SDK and program.

```
torch_market/
├── programs/torch_market/   # Anchor program (Rust)
├── packages/sdk/            # this package
├── docs/                    # whitepaper, architecture, audit, verification
└── ...
```
