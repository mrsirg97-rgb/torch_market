/**
 * Devnet Full E2E Test (v20 — DeepPool AMM)
 *
 * Creates a Flame token (100 SOL target), bonds to completion with many small
 * buys, migrates to DeepPool, then hammers the pool with 200 SOL of post-
 * migration buys/sells routed through TorchVault → vault_swap CPI, plus
 * extended lending and short selling.
 *
 * Run:
 ...
 *   - Torch Market v20 program deployed to devnet
 *   - DeepPool program deployed to devnet
 */
...
// Post-migration config: 200 SOL of buys on DeepPool
...
const main = async () => {
  ...
  // ==================================================================
  // 6. Migrate to DeepPool
  // ==================================================================
  log('\n[6] Migrate to DeepPool')
  try {
    const mintPk = new PublicKey(mint)

    // Auto-migration may have bundled with the last buy
    ...
    // DeepPool reserves: SOL = pool PDA lamports - rent, tokens = vault balance.
    const deepPool = getDeepPoolAccounts(mintPk)
    const readDeepPoolReserves = async (): Promise<{ poolSol: number; poolTokens: number }> => {
      const poolAcct = await connection.getAccountInfo(deepPool.pool)
      const rentExempt = await connection.getMinimumBalanceForRentExemption(POOL_ACCOUNT_SIZE)
      const poolSol = ((poolAcct?.lamports ?? 0) - rentExempt) / LAMPORTS_PER_SOL
      const tokenBal = await connection.getTokenAccountBalance(deepPool.tokenVault)
      const poolTokens = Number(tokenBal.value.amount) / 1e6
      return { poolSol, poolTokens }
    }
    ...
    try {
      ...
      const TREASURY_LOCK = 300_000_000
      const CURVE_SUPPLY = 700_000_000
      const { poolSol, poolTokens } = await readDeepPoolReserves()
    ...
    // DeepPool has no open_time gate — trading is immediate after create_pool.
    ...
    // Pool state snapshot after 200 SOL of vault buys
    try {
      const { poolSol, poolTokens } = await readDeepPoolReserves()
    ...
    // ==================================================================
    // 11. Borrow via vault (post-migration lending)
    // ==================================================================
    log('\n[11] Borrow via vault (post-migration)')
    try {
      const [vaultPda] = getTorchVaultPda(wallet.publicKey)
      const vaultAta = getAssociatedTokenAddressSync(
        new PublicKey(mint),
    ...
    // ==============================================================
    log('\n[14] Short Selling via vault (open_short → close_short)')
    try {
