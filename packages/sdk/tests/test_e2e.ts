...
const main = async () => {
  ...
  if (bondingComplete) {
    ...
    try {
      ...
      const { isWsolToken0, token0Vault: vault0, token1Vault: vault1 } = raydium
      ...
      try {
        ...
        if ((vaultBefore?.sol_balance || 0) < 2) {
          ...
          // Compute how many tokens to borrow to hit ~48% LTV
          // LTV = (tokens * pool_sol / pool_tokens) / sol_collateral
          // tokens = LTV * sol_collateral * pool_tokens / pool_sol
          const raydium = getRaydiumMigrationAccounts(new PublicKey(mint))
          ...
          const poolSol = raydium.isWsolToken0
            ? Number(v0Info.value.amount)
            : Number(v1Info.value.amount)
          const poolTokens = raydium.isWsolToken0
            ? Number(v1Info.value.amount)
            : Number(v0Info.value.amount)
          ...
          // Time travel ~420 days. Interest accrues at 2%/week simple, so at the
          // ~33% open LTV we need ≥48 epochs to clear the 65% threshold; 60
          // gives margin and matches the loan-liquidation test above.
          const FULL_EPOCH_SLOTS2 = 1_512_000 // ~7 days
          const slotsToTravel2 = FULL_EPOCH_SLOTS2 * 60
          log(`  Time traveling ${slotsToTravel2} slots (~420 days)...`)
