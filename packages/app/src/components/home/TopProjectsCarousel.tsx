...
/**
 * Carousel showing top completed projects by market cap
 */
export function TopProjectsCarousel({ tokens }: TopProjectsCarouselProps) {
  ...
  return (
    ...
    >
      ...
      {tokens.length > 0 ? (
        <TokenCardCompact token={tokens[safeIndex]} variant="top" />
      ) : (
        <p className="text-xs" style={{ color: 'var(--muted)' }}>no active markets yet</p>
      )}
      {/* Desktop chevrons */}
      {tokens.length > 1 && (
        <>
